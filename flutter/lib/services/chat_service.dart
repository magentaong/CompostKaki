...
class ChatService {
  ...
  // General methods for admin chat screen (backward compatibility)
  // Note: These methods are deprecated. Use bin-specific methods instead.
  // These are kept for backward compatibility but require binId/userId context.
  Future<List<Map<String, dynamic>>> getMessages({String? binId, String? userId}) async {
    if (binId == null || userId == null) {
      // Return empty list if parameters not provided
      return [];
    }
    return getConversationWithUser(binId, userId);
  }

  RealtimeChannel subscribeToMessages(
      Function(Map<String, dynamic>) onNewMessage, {String? binId}) {
    if (binId == null) {
      // Return a dummy channel that does nothing
      return _supabaseService.client.channel('dummy_channel');
    }
    return subscribeToBinMessages(binId, onNewMessage);
  }

  Future<void> sendMessage(String message, {String? binId, String? receiverId}) async {
    if (binId == null) {
      throw Exception('binId is required for sendMessage');
    }
    return sendBinMessage(binId, message, receiverId: receiverId);
  }
}
